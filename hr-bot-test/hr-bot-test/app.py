...
import config
from database import get_session, User, Vacation, Request, News, Activity, Notification, Reminder, PurchaseExecutor, Broadcast, KnowledgeCategory, KnowledgeArticle, Poll, Onboarding, RequestTemplate, RequestFile, SiteVisit
import json
...
def employees():
    ...
    # Преобразуем объекты User в словари для JSON сериализации
    employees_data = []
    for user in users:
...
def delete_employee(user_id):
    ...
    try:
        ...
        # Удаляем связанные данные через ORM
        # Удаляем задачи онбординга
        db_session.query(Onboarding).filter_by(user_id=user_id).delete()
        
        # Удаляем отпуска
        db_session.query(Vacation).filter_by(user_id=user_id).delete()
        
        # Удаляем заявки
        db_session.query(Request).filter_by(user_id=user_id).delete()
        
        # Удаляем активности
        db_session.query(Activity).filter_by(user_id=user_id).delete()
        
        # Удаляем уведомления
        db_session.query(Notification).filter_by(user_id=user_id).delete()
        
        # Удаляем напоминания (если есть связь с пользователем)
        try:
            db_session.query(Reminder).filter_by(user_id=user_id).delete()
        except:
            pass
        
        # Удаляем записи о зачислении на курсы (если есть)
        try:
            db_session.query(CourseEnrollment).filter_by(user_id=user_id).delete()
        except:
            pass
        
        # Удаляем метрики сотрудника (если есть)
        try:
            db_session.query(EmployeeMetrics).filter_by(user_id=user_id).delete()
        except:
            pass
...
def analytics_data():
    ...
    if hasattr(Request, 'created_at'):
        requests_in_period = db_session.query(Request).filter(
            Request.created_at >= start_date,
            Request.created_at <= end_date
        ).all()
    else:
        requests_in_period = db_session.query(Request).all()
    
    # Метрики
    total_requests = len(requests_in_period)
    approved_requests = len([r for r in requests_in_period if r.status == 'approved'])
    rejected_requests = len([r for r in requests_in_period if r.status == 'rejected'])
    pending_requests = len([r for r in requests_in_period if r.status == 'pending'])
    
    # Новые метрики для API
    today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    visitors_today = db_session.query(SiteVisit.user_id).filter(
    ...
    for i in range(days_in_period, -1, -1):
        ...
        if hasattr(Request, 'created_at'):
            day_requests = [r for r in requests_in_period if day_start <= r.created_at < day_end]
        else:
            day_requests = []
        
        chart_total.append(len(day_requests))
        chart_approved.append(len([r for r in day_requests if r.status == 'approved']))
        chart_rejected.append(len([r for r in day_requests if r.status == 'rejected']))
    
    charts = {
        'requests': {
            'labels': chart_labels,
            'total': chart_total,
            'approved': chart_approved,
        ...
        'types': {
            'labels': ['Отпуск', 'Командировка', 'Больничный'],
            'data': [30, 15, 10]
        },
        'departments': {
            'labels': ['IT', 'HR', 'Sales'],
...
def hr_analytics():
    ...
    rejected_requests = db_session.query(Request).filter_by(status='rejected').count()
    pending_requests = db_session.query(Request).filter_by(status='pending').count()
    active_vacations = db_session.query(Vacation).filter_by(status='approved').count()
    ...
    new_employees_period = db_session.query(User).filter(User.created_at >= thirty_days_ago).count() if hasattr(User, 'created_at') else 0
    
    # Общее количество дней отпуска
    vacation_days_total = db_session.query(Vacation).count() * 14  # Примерно
    
    # Процент одобрения
    approval_rate = round((approved_requests / total_requests * 100) if total_requests > 0 else 0, 1)
    
    # Новые метрики
    # Количество посещений за сегодня (уникальные пользователи)
    today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    visitors_today = db_session.query(SiteVisit.user_id).filter(
        SiteVisit.created_at >= today_start
    ).distinct().count()
    
    # Общее количество посещений за сегодня
    total_visits_today = db_session.query(SiteVisit).filter(
        SiteVisit.created_at >= today_start
    ).count()
    ...
    # Общее количество созданных опросов
    total_polls = db_session.query(Poll).count()
    
    # Активные опросы
    active_polls_count = db_session.query(Poll).filter_by(is_active=True).count()
    ...
    stats = {
        ...
        'new_employees_period': new_employees_period,
    ...
    # Данные для графика заявок (последние 7 дней)
    requests_chart_labels = []
    requests_chart_total = []
    requests_chart_approved = []
    requests_chart_rejected = []
    
    for i in range(6, -1, -1):
        date = datetime.now() - timedelta(days=i)
        date_str = date.strftime('%d.%m')
        requests_chart_labels.append(date_str)
        
        day_start = date.replace(hour=0, minute=0, second=0, microsecond=0)
        day_end = day_start + timedelta(days=1)
        
        total = db_session.query(Request).filter(
            Request.created_at >= day_start,
            Request.created_at < day_end
        ).count() if hasattr(Request, 'created_at') else 0
        
        approved = db_session.query(Request).filter(
            Request.created_at >= day_start,
            Request.created_at < day_end,
            Request.status == 'approved'
        ).count() if hasattr(Request, 'created_at') else 0
        
        rejected = db_session.query(Request).filter(
            Request.created_at >= day_start,
            Request.created_at < day_end,
            Request.status == 'rejected'
        ).count() if hasattr(Request, 'created_at') else 0
        
        requests_chart_total.append(total)
        requests_chart_approved.append(approved)
        requests_chart_rejected.append(rejected)
    
    # Данные для графика отпусков
    vacations_chart_labels = ['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн']
    vacations_chart_data = [5, 8, 12, 7, 10, 6]  # Можно заменить на реальные данные
    
    # Данные для типов заявок
    request_types = {}
    all_requests = db_session.query(Request).all()
    for req in all_requests:
        req_type = req.request_type or 'Другое'
        request_types[req_type] = request_types.get(req_type, 0) + 1
    
    request_types_labels = list(request_types.keys()) if request_types else ['Нет данных']
    request_types_data = list(request_types.values()) if request_types else [0]
    ...
    departments_labels = list(departments.keys()) if departments else ['Нет данных']
    departments_data = list(departments.values()) if departments else [0]
    
    # Данные для активности
    activity_labels = requests_chart_labels
    activity_data = requests_chart_total
    
    # Детальная статистика заявок
    requests_stats = [
        {
            'period': 'Эта неделя',
            'total': len([r for r in all_requests if hasattr(r, 'created_at') and r.created_at >= datetime.now() - timedelta(days=7)]),
            'approved': len([r for r in all_requests if hasattr(r, 'created_at') and r.created_at >= datetime.now() - timedelta(days=7) and r.status == 'approved']),
            'rejected': len([r for r in all_requests if hasattr(r, 'created_at') and r.created_at >= datetime.now() - timedelta(days=7) and r.status == 'rejected']),
            'pending': len([r for r in all_requests if hasattr(r, 'created_at') and r.created_at >= datetime.now() - timedelta(days=7) and r.status == 'pending']),
            'approval_rate': 85
        },
        {
            'period': 'Этот месяц',
    ...
    employee_stats = []
    for user in all_users[:10]:  # Топ 10
        user_requests = db_session.query(Request).filter_by(user_id=user.id).count()
        user_vacations = db_session.query(Vacation).filter_by(user_id=user.id).count()
        
        employee_stats.append({
            'name': f"{user.first_name} {user.last_name or ''}",
            'initials': user.first_name[0] if user.first_name else 'U',
            'department': user.department,
            'requests_count': user_requests,
            'vacations_count': user_vacations,
            'points': user.points or 0
        })
    ...
    return render_template('hr_analytics.html',
                         stats=stats,
                         requests_chart_labels=json.dumps(requests_chart_labels),
                         requests_chart_total=json.dumps(requests_chart_total),
                         requests_chart_approved=json.dumps(requests_chart_approved),
                         requests_chart_rejected=json.dumps(requests_chart_rejected),
                         vacations_chart_labels=json.dumps(vacations_chart_labels),
                         vacations_chart_data=json.dumps(vacations_chart_data),
                         request_types_labels=json.dumps(request_types_labels),
                         request_types_data=json.dumps(request_types_data),
                         departments_labels=json.dumps(departments_labels),
                         departments_data=json.dumps(departments_data),
...
def add_file_link():
    ...
    try:
        ...
        request_file = RequestFile(
            ...
            file_path=file_url,  # Используем URL в качестве пути
            file_url=file_url,
            file_type=file_type,
            company=company,
